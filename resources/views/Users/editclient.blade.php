<x-guest-layout>
    ...
    <div class="py-12 px-3 rounded-lg  bg-gray-50">
            ...
            <div class="bg-primary shadow-sm sm:rounded-lg text-white">
                <div class="p-6 text-gray-900">
                    ...
                    <form method="POST" action="{{ route('users.update', $user) }}" enctype="multipart/form-data">
                        ...
                        <!-- Photo de profil -->
                        <div class="m-4">
                            <label for="avatar" class="block text-sm font-medium text-white text-xl">Photo de profil</label>
                            <input type="file" name="avatar" id="avatar"
                                class="mt-1 p-2 block w-full rounded-lg border-gray-300"
                                onchange="previewImages(event)">
                            <div id="image-preview-container" class="grid grid-cols-3 sm:grid-cols-6 gap-4 mt-4"></div>
                            @error('avatar')
                                ...
                                <select name="gender" id="gender"
                                    ...
                                    @foreach (App\Enums\GenderEnum::cases() as $gender)
                                        <option value="{{ $gender->value }}" {{ $user->gender == $gender->value ? 'selected' : '' }}>
                                            @if ($gender->value == 'male') Homme
                                            @elseif ($gender->value == 'female') Femme @endif
                                        </option>
                                    @endforeach
                                </select>
                                @error('gender')
                        ...
                        <button type="submit" class="w-full py-3 px-6 bg-white text-primary text-lg rounded-full mt-4">
                            Enregistrer
                        </button>
                    </form>
                </div>
    ...
    function previewImages(event) {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            reader.onload = function(e) {
                ...
                deleteButton.classList.add('flex', 'items-center', 'justify-center', 'w-8', 'h-8', 'bg-red-600', 'text-white', 'rounded-full', 'cursor-pointer');
